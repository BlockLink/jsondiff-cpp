...
namespace fjson {
  ...
  class variant;
  void to_variant( const fjson::microseconds&,  fjson::variant&  );
  void from_variant( const fjson::variant& , fjson::microseconds& );
  ...
  class time_point {
    public:
        ...
        operator fjson::string()const;
        static time_point from_iso_string( const fjson::string& s );
  ...
  class time_point_sec
  {
    public:
        ...
        operator time_point()const { return time_point( fjson::seconds( utc_seconds) ); }
        uint32_t sec_since_epoch()const { return utc_seconds; }

        time_point_sec operator = ( const fjson::time_point& t )
        {
          utc_seconds = t.time_since_epoch().count() / 1000000ll;
          return *this;
        }
        ...
        fjson::string to_non_delimited_iso_string()const;
        fjson::string to_iso_string()const;

        operator fjson::string()const;
        static time_point_sec from_iso_string( const fjson::string& s );

    private:
        uint32_t utc_seconds;
  };

  typedef fjson::optional<time_point> otime_point;
...
FJSON_REFLECT_TYPENAME( fjson::time_point )
FJSON_REFLECT_TYPENAME( fjson::microseconds )
FJSON_REFLECT_TYPENAME( fjson::time_point_sec )
