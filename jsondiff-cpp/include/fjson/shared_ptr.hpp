...
namespace fjson {
  ...
  fjson::shared_ptr<T> dynamic_pointer_cast( const fjson::shared_ptr<O>& t ) {
    return fjson::shared_ptr<T>( dynamic_cast<T*>(t.get()), true );
  }
  template<typename T, typename O>
  fjson::shared_ptr<T> static_pointer_cast( const fjson::shared_ptr<O>& t ) {
    return fjson::shared_ptr<T>( static_cast<T*>(t.get()), true );
  }
}
