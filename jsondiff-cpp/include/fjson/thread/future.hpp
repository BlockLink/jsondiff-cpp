...
//#define FJSON_TASK_NAMES_ARE_MANDATORY 1
#ifdef FJSON_TASK_NAMES_ARE_MANDATORY
# define FJSON_TASK_NAME_DEFAULT_ARG
#else
# define FJSON_TASK_NAME_DEFAULT_ARG = "?"
#endif

//#define FJSON_CANCELATION_REASONS_ARE_MANDATORY 1
#ifdef FJSON_CANCELATION_REASONS_ARE_MANDATORY
# define FJSON_CANCELATION_REASON_DEFAULT_ARG
#else
# define FJSON_CANCELATION_REASON_DEFAULT_ARG = nullptr
#endif
...
namespace fjson {
  ...
  namespace detail {
     class completion_handler {
       public:
          ...
          virtual void on_complete( const void* v, const fjson::exception_ptr& e ) = 0;
     };
     
     template<typename Functor, typename T>
     class completion_handler_impl : public completion_handler {
       public:
         completion_handler_impl( Functor&& f ):_func(fjson::move(f)){}
         completion_handler_impl( const Functor& f ):_func(f){}
     
         virtual void on_complete( const void* v, const fjson::exception_ptr& e ) {
  ...
  class promise_base : public virtual retainable{
    public:
      typedef fjson::shared_ptr<promise_base> ptr;
      promise_base(const char* desc FJSON_TASK_NAME_DEFAULT_ARG);

      const char* get_desc()const;
                   
      virtual void cancel(const char* reason FJSON_CANCELATION_REASON_DEFAULT_ARG);
      bool canceled()const { return _canceled; }
      bool ready()const;
      bool error()const;

      void set_exception( const fjson::exception_ptr& e );
  ...
  class promise : virtual public promise_base {
    public:
      typedef fjson::shared_ptr< promise<T> > ptr;
      promise( const char* desc FJSON_TASK_NAME_DEFAULT_ARG):promise_base(desc){}
      promise( const T& val ){ set_value(val); }
      promise( T&& val ){ set_value(fjson::move(val) ); }
      ...
      void on_complete( CompletionHandler&& c ) {
        _on_complete( new detail::completion_handler_impl<CompletionHandler,T>(fjson::forward<CompletionHandler>(c)) );
      }
    protected:
      optional<T> result;
      ~promise(){}
  ...
  class promise<void> : virtual public promise_base {
    public:
      typedef fjson::shared_ptr< promise<void> > ptr;
      promise( const char* desc FJSON_TASK_NAME_DEFAULT_ARG):promise_base(desc){}
      ...
      template<typename CompletionHandler>
      void on_complete( CompletionHandler&& c ) {
        _on_complete( new detail::completion_handler_impl<CompletionHandler,void>(fjson::forward<CompletionHandler>(c)) );
      }
    protected:
      ~promise(){}
  };
  ...
  class future {
    public:
      future( const fjson::shared_ptr<promise<T>>& p ):m_prom(p){}
      future( fjson::shared_ptr<promise<T>>&& p ):m_prom(fjson::move(p)){}
      future(const future<T>& f ) : m_prom(f.m_prom){}
      future(){}

      future& operator=(future<T>&& f ) {
        fjson_swap(m_prom,f.m_prom); 
        return *this;
      }
      ...
      void cancel(const char* reason FJSON_CANCELATION_REASON_DEFAULT_ARG) const { if( m_prom ) m_prom->cancel(reason); }
      bool canceled()const { if( m_prom ) return m_prom->canceled(); else return true;}

      void cancel_and_wait(const char* reason FJSON_CANCELATION_REASON_DEFAULT_ARG)
      {
         if( valid() )
         {
            cancel(reason);
      ...
      template<typename CompletionHandler>
      void on_complete( CompletionHandler&& c ) {
        m_prom->on_complete( fjson::forward<CompletionHandler>(c) );
      }
    private:
      friend class thread;
      fjson::shared_ptr<promise<T>> m_prom;
  };

  template<>
  class future<void> {
    public:
      future( const fjson::shared_ptr<promise<void>>& p ):m_prom(p){}
      future( fjson::shared_ptr<promise<void>>&& p ):m_prom(fjson::move(p)){}
    ...
    private:
      friend class thread;
      fjson::shared_ptr<promise<void>> m_prom;
  };
} 
