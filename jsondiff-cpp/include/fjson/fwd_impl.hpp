...
namespace fjson {
    ...
    auto operator + ( const fwd<T,S,A>& x, U&& u ) -> typename detail::add<T,U>::type { return *x+fjson::forward<U>(u); }

    template<typename T, unsigned int S, typename U, typename A>
    auto operator - ( const fwd<T,S,A>& x, U&& u ) -> typename detail::sub<T,U>::type { return *x-fjson::forward<U>(u); }
    ...
    void check_size() { static_assert( (ProvidedSize >= RequiredSize), "Failed to reserve enough space in fjson::fwd<T,S>" ); }

    template<typename T,unsigned int S,typename A>
    template<typename U>
    fwd<T,S,A>::fwd( U&& u ) {
      check_size<sizeof(T),sizeof(_store)>();
      new (this) T( fjson::forward<U>(u) );
    }
    ...
    fwd<T,S,A>::fwd( U&& u, V&& v ) {
      check_size<sizeof(T),sizeof(_store)>();
      new (this) T( fjson::forward<U>(u), fjson::forward<V>(v) );
    }
    template<typename T,unsigned int S,typename A>
    template<typename U,typename V,typename X,typename Y>
    fwd<T,S,A>::fwd( U&& u, V&& v, X&& x, Y&&  y ) {
      check_size<sizeof(T),sizeof(_store)>();
      new (this) T( fjson::forward<U>(u), fjson::forward<V>(v), fjson::forward<X>(x), fjson::forward<Y>(y) );
    }
