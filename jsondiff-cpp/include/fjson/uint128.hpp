...
namespace fjson
{
  ...
  class uint128
  {
     ...
     public:
      ...
      :hi(_h),lo(_l){}
      uint128( const fjson::bigint& bi );

      operator std::string()const;
      operator fjson::bigint()const;
...
{
    template<>
    struct hash<fjson::uint128>
    {
       size_t operator()( const fjson::uint128& s )const
       {
           return fjson::city_hash_size_t((char*)&s, sizeof(s));
       }
    };
}

FJSON_REFLECT( fjson::uint128_t, (hi)(lo) )
