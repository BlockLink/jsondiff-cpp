...
namespace fjson
{
   ...
   template<typename K, typename T>
   void to_variant( const fjson::flat_map<K,T>& var,  variant& vo );
   template<typename K, typename T>
   void from_variant( const variant& var, fjson::flat_map<K,T>& vo );
   ...
   template<typename T>
   void to_variant( const fjson::flat_set<T>& var,  variant& vo );
   template<typename T>
   void from_variant( const variant& var, fjson::flat_set<T>& vo );
   ...
   class variant
   {
      public:
       ...
       ~variant();
        ...
        {
           return *this = variant( fjson::forward<T>(v) );
        }
...
FJSON_REFLECT_TYPENAME( fjson::variant )
FJSON_REFLECT_ENUM( fjson::variant::type_id, (null_type)(int64_type)(uint64_type)(double_type)(bool_type)(string_type)(array_type)(object_type)(blob_type) )
FJSON_REFLECT( fjson::blob, (data) );
