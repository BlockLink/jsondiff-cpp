...
namespace fjson
{
   FJSON_REGISTER_EXCEPTIONS( (timeout_exception)
   ...
   :my( new detail::exception_impl() )
   {
      ...
      my->_what = what_value;
      my->_name = name_value;
      my->_elog.push_back( fjson::move( msg ) );
   }
   ...
   {
      my->_elog.emplace_back( fjson::move(m) );
   }
   ...
   string exception::to_detail_string( log_level ll  )const
   {
      ...
      {
         ss << itr->get_message() <<"\n"; //fjson::format_string( itr->get_format(), itr->get_data() ) <<"\n";
   ...
   string exception::to_string( log_level ll   )const
   {
      ...
      {
         ss << fjson::format_string( itr->get_format(), itr->get_data() ) <<"\n";
   //      ss << "    " << itr->get_context().to_string() <<"\n";
      }
      return ss.str();
   }
   ...
   {
      FJSON_THROW_EXCEPTION( bad_cast_exception,
   ...
   {
      fjson::mutable_variant_object assert_trip_info =
         fjson::mutable_variant_object()
         ("source_file", filename)
         ("source_lineno", lineno)
         ("expr", expr)
         ;
      std::cout
         << "FJSON_ASSERT triggered:  "
         << fjson::json::to_string( assert_trip_info ) << "\n";
      return;
   }
