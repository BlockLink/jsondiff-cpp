...
namespace fjson {
  ...
  fjson::string time_point_sec::to_non_delimited_iso_string()const
  {
    const auto ptime = boost::posix_time::from_time_t( time_t( sec_since_epoch() ) );
    return boost::posix_time::to_iso_string( ptime );
  }

  fjson::string time_point_sec::to_iso_string()const
  {
    const auto ptime = boost::posix_time::from_time_t( time_t( sec_since_epoch() ) );
    return boost::posix_time::to_iso_extended_string( ptime );
  }

  time_point_sec::operator fjson::string()const
  {
      return this->to_iso_string();
  }

  time_point_sec time_point_sec::from_iso_string( const fjson::string& s )
  { try {
      ...
      return fjson::time_point_sec( (pt - epoch).total_seconds() );
  } FJSON_RETHROW_EXCEPTIONS( warn, "unable to convert ISO-formatted string to fjson::time_point_sec" ) }

  time_point::operator fjson::string()const
  {
      return fjson::string( time_point_sec( *this ) );
  }

  time_point time_point::from_iso_string( const fjson::string& s )
  { try {
      return time_point( time_point_sec::from_iso_string( s ) );
  } FJSON_RETHROW_EXCEPTIONS( warn, "unable to convert ISO-formatted string to fjson::time_point" ) }

  void to_variant( const fjson::time_point& t, variant& v ) {
    v = fjson::string( t );
  }
  void from_variant( const fjson::variant& v, fjson::time_point& t ) {
    t = fjson::time_point::from_iso_string( v.as_string() );
  }
  void to_variant( const fjson::time_point_sec& t, variant& v ) {
    v = fjson::string( t );
  }
  void from_variant( const fjson::variant& v, fjson::time_point_sec& t ) {
    t = fjson::time_point_sec::from_iso_string( v.as_string() );
  }
  ...
  string get_approximate_relative_time_string(const time_point_sec& event_time,
                                              const time_point_sec& relative_to_time /* = fjson::time_point::now() */,
  ...
  string get_approximate_relative_time_string(const time_point& event_time,
                                              const time_point& relative_to_time /* = fjson::time_point::now() */,
