...
namespace fjson  {
...
#ifdef USE_FJSON_STRING
  string::string(const char* s, int l) :my(s,l){ }
  string::string(){}
  string::string( const fjson::string& c ):my(*c.my) { }
  string::string( string&& m ):my(fjson::move(*m.my)) {}
  ...
  string::string( std::string&& s ):my(fjson::move(s)) {}
  string::~string() { }
  ...
  size_t   string::find(const fjson::string& str, size_t pos /* = 0 */) const { return my->find(str, pos); }
  ...
  size_t   string::rfind(const fjson::string& c, size_t p)const { return my->rfind(c,p); }
  size_t   string::find_first_of(const fjson::string& str, size_t pos /* = 0 */) const { return my->find_first_of(str, pos); }
  size_t   string::find_first_of(const char* s, size_t pos /* = 0 */) const { return my->find_first_of(s, pos); }

  fjson::string& string::replace(size_t pos,  size_t len,  const string& str) { my->replace(pos, len, str); return *this; }
  fjson::string& string::replace(size_t pos,  size_t len,  const char* s) { my->replace(pos, len, s); return *this; }

  void       string::clear()                       { my->clear(); }
  void       string::resize( size_t s )          { my->resize(s); }
                                            
  fjson::string string::substr( size_t start, size_t len )const { return my->substr(start,len); }
  ...
  string& string::operator =( string&& c )               { *my = fjson::move( *c.my ); return *this; }
...
#endif // USE_FJSON_STRING


  int64_t    to_int64( const fjson::string& i )
  {
    try
    {
    ...
    {
      FJSON_THROW_EXCEPTION( parse_error_exception, "Couldn't parse int64_t" );
    }
    FJSON_RETHROW_EXCEPTIONS( warn, "${i} => int64_t", ("i",i) )
  }

  uint64_t   to_uint64( const fjson::string& i )
  { try {
    try
    {
    ...
    {
      FJSON_THROW_EXCEPTION( parse_error_exception, "Couldn't parse uint64_t" );
    }
    FJSON_RETHROW_EXCEPTIONS( warn, "${i} => uint64_t", ("i",i) )
  } FJSON_CAPTURE_AND_RETHROW( (i) ) }

  double     to_double( const fjson::string& i)
  {
    try
    {
    ...
    {
      FJSON_THROW_EXCEPTION( parse_error_exception, "Couldn't parse double" );
    }
    FJSON_RETHROW_EXCEPTIONS( warn, "${i} => double", ("i",i) )
  }

  fjson::string to_string(double d)
  {
  ...
  fjson::string to_string( uint64_t d)
  {
    return boost::lexical_cast<std::string>(d);
  }

  fjson::string to_string( int64_t d)
  {
    return boost::lexical_cast<std::string>(d);
  }
  fjson::string to_string( uint16_t d)
  {
    return boost::lexical_cast<std::string>(d);
  }
