...
namespace fjson
{
   ...
   variant_object::entry::entry( string k, variant v ) : _key(fjson::move(k)),_value(fjson::move(v)) {}
   variant_object::entry::entry( entry&& e ) : _key(fjson::move(e._key)),_value(fjson::move(e._value)) {}
   ...
   {
      fjson_swap( _key, e._key );
      fjson_swap( _value, e._value );
      return *this;
   }
   ...
   const variant& variant_object::operator[]( const char* key )const
   {
      ...
      FJSON_THROW_EXCEPTION( key_not_found_exception, "Key ${key}", ("key",key) );
   }
   ...
   variant_object::variant_object( string key, variant val )
      : _key_value(std::make_shared<std::vector<entry>>())
   {
       //_key_value->push_back(entry(fjson::move(key), fjson::move(val)));
       _key_value->emplace_back(entry(fjson::move(key), fjson::move(val)));
   }
   ...
   : _key_value( fjson::move(obj._key_value) )
   {
   ...
   variant_object& variant_object::operator=( variant_object&& obj )
   {
      if (this != &obj)
      {
         fjson_swap(_key_value, obj._key_value );
         assert( _key_value != nullptr );
      }
      return *this;
   }
   ...
   mutable_variant_object& mutable_variant_object::set( string key, variant var )
   {
      ...
      if( itr != end() )
      {
         itr->set( fjson::move(var) );
      }
      else
      {
         _key_value->push_back( entry( fjson::move(key), fjson::move(var) ) );
      }
      return *this;
   }
