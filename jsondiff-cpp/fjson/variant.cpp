...
#include <fjson/reflect/variant.hpp>
#include <algorithm>
...
variant::variant( fjson::nullptr_t )
{
   set_variant_type( this, null_type );
}
...
variant::variant( fjson::string val )
{
   *reinterpret_cast<string**>(this)  = new string( fjson::move(val) );
   set_variant_type( this, string_type );
}
variant::variant( blob val )
{
   *reinterpret_cast<blob**>(this)  = new blob( fjson::move(val) );
   set_variant_type( this, blob_type );
}

variant::variant( variant_object obj)
{
   *reinterpret_cast<variant_object**>(this)  = new variant_object(fjson::move(obj));
...
{
   *reinterpret_cast<variants**>(this)  = new variants(fjson::move(arr));
...
void  variant::visit( const visitor& v )const
{
   switch( get_type() )
   {
      ...
      case object_type:
         ...
         return;
      default:
         FJSON_THROW_EXCEPTION( assert_exception, "Invalid Type / Corrupted Memory" );
   }
}
...
int64_t variant::as_int64()const
{
   switch( get_type() )
   {
      ...
      case null_type:
          return 0;
      default:
         FJSON_THROW_EXCEPTION( bad_cast_exception, "Invalid cast from ${type} to int64", ("type", get_type()) );
   }
}
...
{ try {
   switch( get_type() )
   {
      ...
      case null_type:
          return 0;
      default:
         FJSON_THROW_EXCEPTION( bad_cast_exception,"Invalid cast from ${type} to uint64", ("type",get_type()));
   }
} FJSON_CAPTURE_AND_RETHROW( (*this) ) }
...
double  variant::as_double()const
{
   switch( get_type() )
   {
      ...
      case null_type:
          return 0;
      default:
         FJSON_THROW_EXCEPTION( bad_cast_exception, "Invalid cast from ${type} to double", ("type",get_type()) );
   }
}
...
bool  variant::as_bool()const
{
   switch( get_type() )
   {
      case string_type:
      {
          ...
          if( s == "false" )
             return false;
          FJSON_THROW_EXCEPTION( bad_cast_exception, "Cannot convert string to bool (only \"true\" or \"false\" can be converted)" );
      }
      case double_type:
      ...
      case null_type:
          return false;
      default:
         FJSON_THROW_EXCEPTION( bad_cast_exception, "Invalid cast from ${type} to bool" , ("type",get_type()));
   }
}
...
string    variant::as_string()const
{
   switch( get_type() )
   {
      ...
      case null_type:
          return string();
      default:
      FJSON_THROW_EXCEPTION( bad_cast_exception, "Invalid cast from ${type} to string", ("type", get_type() ) );
   }
}
...
variants&         variant::get_array()
{
  ...
  FJSON_THROW_EXCEPTION( bad_cast_exception, "Invalid cast from ${type} to Array", ("type",get_type()) );
}
blob&         variant::get_blob()
{
  ...
  FJSON_THROW_EXCEPTION( bad_cast_exception, "Invalid cast from ${type} to Blob", ("type",get_type()) );
}
...
variant_object&        variant::get_object()
{
  ...
  FJSON_THROW_EXCEPTION( bad_cast_exception, "Invalid cast from ${type} to Object", ("type",get_type()) );
}
...
const string&        variant::get_string()const
{
  if( get_type() == string_type )
     return **reinterpret_cast<const const_string_ptr*>(this);
  FJSON_THROW_EXCEPTION( bad_cast_exception, "Invalid cast from type '${type}' to Object", ("type",get_type()) );
}
...
string      format_string( const string& format, const variant_object& args )
{
   ...
   variant operator < ( const variant& a, const variant& b )
   {
      ...
      if( a.is_int64()   || b.is_int64() )  return a.as_int64() < b.as_int64();
      if( a.is_uint64()  || b.is_uint64() ) return a.as_uint64() < b.as_uint64();
      FJSON_ASSERT( false, "Invalid operation" );
   }
   ...
   variant operator + ( const variant& a, const variant& b )
   {
      ...
      if( a.is_int64()   || b.is_int64() )  return a.as_int64() + b.as_int64();
      if( a.is_uint64()  || b.is_uint64() ) return a.as_uint64() + b.as_uint64();
      FJSON_ASSERT( false, "invalid operation ${a} + ${b}", ("a",a)("b",b) );
   }
