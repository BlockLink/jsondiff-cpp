#include <fjson/real128.hpp>
#include <fjson/crypto/bigint.hpp>
...
namespace fjson
{
   ...
   { 
      return (fixed/ FJSON_REAL128_PRECISION).to_uint64(); 
   }

   real128::real128( uint64_t integer )
   {
      fixed = uint128(integer) * FJSON_REAL128_PRECISION;
   }
   ...
   real128& real128::operator /= ( const real128& o )
   { try {
      FJSON_ASSERT( o.fixed > uint128(0), "Divide by Zero" );
       
      fjson::bigint self(fixed);
      fjson::bigint other(o.fixed);
      self *= FJSON_REAL128_PRECISION;
      self /= other;
      fixed = self;

      return *this;
   } FJSON_CAPTURE_AND_RETHROW( (*this)(o) ) }
   ...
   {
      std::stringstream ss;
      ss << std::string(fixed / FJSON_REAL128_PRECISION);
      ss << '.';
      auto frac = (fixed % FJSON_REAL128_PRECISION) + FJSON_REAL128_PRECISION;
